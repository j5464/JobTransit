...
def get_job_id():
    ...
    # 經討論，每天撈取的話，不需要讀取舊有的job_id，因為每天都會重新撈取，且只要今天的職缺就好，所以不需要比對舊有的資料。
    # --- 讀取 Mongo DB --- 
    # jobs_dict = get_existing_job_ids()
    # --------------
    jobs_dict = {}  # 用於存放當天撈取的職缺 ID 與狀態
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for job in target_jobs:
                        ...
                        job_date = appear_date_iso.date()

                        # 2. 遇到非今日職缺（舊資料），標記停止並跳出迴圈
                        if job_date != today_date:
                            print(f"遇到非今日職缺 (更新日期: {job_date})，結束 cat:{jobcat} 撈取。")
                            stop_current_cat = True
                            break
                    ...
                    # 可以再研究:如果 airflow同一天重啟的話，可以如何讓 job_id 不會重複寫入
